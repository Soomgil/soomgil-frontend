...
it('shows five ranked results, expands every result, and returns to overview', async () => {
 ...
 expect(wrapper.findAll('.is-top-three')).toHaveLength(3)
 expect(wrapper.findAll('.is-runner-up')).toHaveLength(2)
 expect(wrapper.findAll('[data-testid="result-row"]')[3].classes()).not.toContain('is-top-three')
 expect(wrapper.findAll('[data-testid="result-row"]')[4].classes()).not.toContain('is-top-three')
 expect(wrapper.findAll('.is-compact')).toHaveLength(4)
