...
it('loads only when opened, filters actual reactions and clears markers when closed', async () => {
  ...
  expect(w.findAll('[data-testid="taste-place"]')).toHaveLength(2)
  await w.get('[data-testid="taste-together"]').trigger('click')
  expect(w.findAll('[data-testid="taste-place"]')).toHaveLength(1)
  await w.get('[data-testid="taste-colleagues"]').trigger('click')
  expect(w.findAll('[data-testid="taste-place"]')).toHaveLength(1)
  await w.get('[data-testid="taste-place"]').trigger('click')
  ...
  expect(w.findAll('[data-testid="taste-place"]')).toHaveLength(0)
  await w.get('[data-testid="taste-toggle"]').trigger('click')
  expect(w.emitted('places')?.at(-1)?.[0]).toEqual([])
  w.unmount()
})
it('discards a response that arrives after closing the panel', async () => {
